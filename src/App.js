// eslint-disable-next-line no-unused-varsimport React,{useState,useEffect,useRef,useCallback}from"react";
...
function BazaarMode({baz,data,update,T,cur,catIcon,cssVars,fScale,onExit}){
  ...
  const confirmSale=()=>{
    ...
    const sales=cart.map((item,i)=>{
      ...
      return{id:saleId+"-"+String(i),productId:item.id,productName:item.name,bazaarId:baz.id,qty:item.qty,unitPrice:Math.round(unitPrice),total:Math.round(total),totalProfit:Math.round(profit),channel:"بازار",date:todayStr(),unitCost:item.totalCost,isBundle:cart.length>1};
    });
...
function Dashboard({stats,data,cur,catIcon,T,setTab,alerts,setBazaarMode}){
  const getDashKey=s=>{if(s.createdAt&&Number(s.createdAt)>1000000000000)return Number(s.createdAt);const n=Number(s.id);if(!isNaN(n)&&n>1000000000000)return n;const p=(s.date||"").split("/");if(p.length===3&&p[2].length===4)return new Date(p[2]+"-"+p[1].padStart(2,"0")+"-"+p[0].padStart(2,"0")).getTime();return 0;};
  const recent=[...data.sales].sort((a,b)=>getDashKey(b)-getDashKey(a)).slice(0,5);
...
function Products({data,update,cur,hr,catIcon,T}){
  ...
  const addReady=(id,count)=>update(prev=>{
    ...
    if(prod?.materialUsage){
      prod.materialUsage.forEach(u=>{
        const mi=materials.findIndex(m=>m.id===u.materialId);
        if(mi<0)return;
        if((materials[mi].itemType||"consumable")==="asset")return; // assets not deducted
...
function Sales({data,update,cur,catIcon,T}){
  ...
  const save=()=>{
    ...
    else{const saleTs=Date.now();update(prev=>({...prev,products:prev.products.map(p=>p.id===form.productId?{...p,readyCount:Math.max(0,(Number(p.readyCount)||0)-qty),soldCount:(Number(p.soldCount)||0)+qty}:p),sales:[...prev.sales,{...form,id:saleTs.toString(),createdAt:saleTs,productName:prod?.name||"",unitPrice,total,totalProfit:profit,qty,date:todayStr(),unitCost:prod?.totalCost||0}]}));}
  ...
  const parseSaleDate=d=>{if(!d)return 0;const p=d.split("/");if(p.length===3&&p[2].length===4)return new Date(p[2]+"-"+p[1].padStart(2,"0")+"-"+p[0].padStart(2,"0")).getTime();if(d.includes("-"))return new Date(d).getTime()||0;return 0;};
  const getSaleKey=s=>{if(s.createdAt&&Number(s.createdAt)>1000000000000)return Number(s.createdAt);const n=Number(s.id);if(!isNaN(n)&&n>1000000000000)return n;return parseSaleDate(s.date);};
  const sorted=[...data.sales].sort((a,b)=>sortDir==="asc"?getSaleKey(a)-getSaleKey(b):getSaleKey(b)-getSaleKey(a));
  const grouped={};
  sorted.forEach(s=>{const d=s.date||"بدون تاريخ";if(!grouped[d])grouped[d]=[];grouped[d].push(s);});
  const dayKeys={};
  Object.keys(grouped).forEach(day=>{
    const keys=grouped[day].map(s=>getSaleKey(s));
    dayKeys[day]=sortDir==="asc"?Math.min.apply(null,keys):Math.max.apply(null,keys);
  });
  const days=Object.keys(grouped).sort((a,b)=>sortDir==="asc"?dayKeys[a]-dayKeys[b]:dayKeys[b]-dayKeys[a]);
  return(
    <div>
...
function ItemTypeBadge({itemType,T}){
  const t=itemType||"consumable";
  const bg=t==="asset"?T.blueFaint:t==="packaging"?T.yellowFaint:T.accentVeryFaint;
  const color=t==="asset"?T.blue:t==="packaging"?T.yellow:T.accent;
  const label=t==="asset"?"♻️ أصل":t==="packaging"?"📦 تغليف":"🧵 مستهلك";
  return <span style={{fontSize:9,fontWeight:600,padding:"2px 6px",borderRadius:5,background:bg,color:color}}>{label}</span>;
}

function MergeHint({name,materials,nameSug,selectedMatId,T}){
  if(nameSug.length>0) return null;
  // Find by ID first, then by name
  const norm=s=>(s||"").trim().replace(/\s+/g," ").toLowerCase();
  let existing=selectedMatId?materials.find(m=>m.id===selectedMatId):null;
  if(!existing&&name&&name.trim()){
    existing=materials.find(m=>norm(m.name)===norm(name));
  }
  if(!existing||!name||!name.trim()) return null;
  return(
    <div style={{marginTop:6,padding:"9px 11px",background:T.greenFaint,borderRadius:9,fontSize:12,color:T.green,border:`1px solid ${T.green}30`,lineHeight:1.7}}>
      <div style={{fontWeight:700,marginBottom:2}}>✅ ريستوك — مادة موجودة</div>
      <div>الرصيد الحالي: <strong>{fmt(existing.quantity)} {existing.unit}</strong></div>
      <div style={{fontSize:10,color:T.textFaint,marginTop:2}}>الكمية الجديدة ستُضاف · السعر الجديد سيُطبَّق</div>
    </div>
  );
}
...
function Inventory({data,update,cur,T}){
  ...
  // selectedMatId: stores the ID of existing material chosen from suggestions
  // This is the KEY fix — we match by ID (reliable) not just name string
  const[selectedMatId,setSelectedMatId]=useState(null);
  const ef=()=>({name:"",unit:"غرام",totalPurchasePrice:"",quantity:"",minAlert:10,itemType:"consumable"});
  const[form,setForm]=useState(ef);
  const[nameSug,setNameSug]=useState([]);

  // Normalize name for comparison: trim + collapse spaces + lowercase
  const norm=s=>(s||"").trim().replace(/\s+/g," ").toLowerCase();

  const handleName=val=>{
    setForm(f=>({...f,name:val}));
    setSelectedMatId(null); // clear selected ID when user types manually
    if(val.trim().length>0){
      const matches=data.materials.filter(m=>norm(m.name).includes(norm(val)));
      setNameSug(matches.slice(0,6));
    } else setNameSug([]);
  };

  const selectSug=m=>{
    // Store the actual ID of the chosen material — this is reliable
  ...
  const add=()=>{
    ...
    update(prev=>{
      // STEP 1: Find existing material
      // Priority: by ID (user picked from list) → by normalized name (user typed)
      let ei=-1;
      if(selectedMatId){
        ei=prev.materials.findIndex(m=>m.id===selectedMatId);
      }
      if(ei<0){
        // Fallback: match by normalized name
        const normalizedInput=norm(form.name);
        ei=prev.materials.findIndex(m=>norm(m.name)===normalizedInput);
      }

      let mats;
      if(ei>=0){
        // ✅ RESTOCK: found existing material → add qty, update price
        ...
        mats=prev.materials.map((m,i)=>i===ei?{
          ...m,
          quantity:mergedQty,       // ADD to existing
          costPerUnit:finalCpu,     // use new price per unit
          costPer100:finalCpu*100,
          totalCost:newTotalPrice>0?newTotalPrice:m.totalCost,
        }:m);
      } else {
        // 🆕 NEW material — create permanent record
        mats=[...prev.materials,{
      ...
      {showAdd&&!editMat&&<div style={{...CDS(T),marginBottom:12}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          ...
          <div style={{gridColumn:"span 2"}}>
            <LB T={T}>نوع العنصر</LB>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6}}>
              {[
                {val:"consumable",label:"مستهلك 🧵",sub:"يُخصم عند الإنتاج"},
                {val:"asset",label:"أصل ♻️",sub:"لا يُخصم (قوالب...)"},
                {val:"packaging",label:"تغليف 📦",sub:"يُخصم عند الاستخدام"},
              ].map(t=>(
                <button key={t.val} type="button" onClick={()=>setForm(f=>({...f,itemType:t.val}))} style={{
                  background:(form.itemType||"consumable")===t.val?T.primary:T.card,
                  border:`1px solid ${(form.itemType||"consumable")===t.val?"transparent":T.cardBorder}`,
                  borderRadius:9,padding:"7px 4px",cursor:"pointer",
                  color:(form.itemType||"consumable")===t.val?"#fff":T.text,
                  fontFamily:"inherit",fontSize:11,fontWeight:(form.itemType||"consumable")===t.val?700:400,
        ...
        <MergeHint name={form.name} materials={data.materials} nameSug={nameSug} selectedMatId={selectedMatId} T={T}/>
          ...
          <div key={m.id} style={{...CDS(T),border:low?`1px solid ${T.red}45`:undefined}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:3}}>
              <div>
                ...
                <ItemTypeBadge itemType={m.itemType} T={T}/>
              </div>
